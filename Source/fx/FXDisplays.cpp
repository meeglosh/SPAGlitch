...
OttDisplay::OttDisplay (juce::AudioProcessorValueTreeState& state,
...
void OttDisplay::timerCallback()
{
    for (int b = 0; b < (int) shown.size(); ++b)
    {
        const auto target = readBandGain ? readBandGain (b) : 0.0f;
        // Snap toward a bigger excursion and ease back from it, so the bar
...
void OttDisplay::paint (juce::Graphics& g)
{
    auto bounds = getLocalBounds().toFloat();
    g.setColour (well.withAlpha (0.85f));
    g.fillRoundedRectangle (bounds, 7.0f);
    ...
    auto* enableParam = apvts.getParameter (params::id::ottEnable);
    const auto on = enableParam != nullptr && enableParam->getValue() >= 0.5f;
    const auto boostColour = on ? electric : muted.withAlpha (0.4f);
    const auto cutColour   = on ? sage : muted.withAlpha (0.4f);

    auto area = bounds.reduced (10.0f, 8.0f);
    auto labelRow = area.removeFromBottom (20.0f);

    // Symmetrical scale: the bar is clipped rather than rescaled, so a band
    // pinned at the extreme looks pinned instead of quietly renormalising the
    // whole meter.
    constexpr float rangeDb = 18.0f;
    const auto centreY = area.getCentreY();
    const auto halfHeight = area.getHeight() * 0.5f;

    // Scale lines, so a moving bar can be read as a number of dB rather than
    // just as movement. Without them the panel says nothing at all at rest.
    g.setFont (juce::Font (juce::FontOptions (8.0f)));
    for (const int markDb : { 12, 6 })
    {
        const auto offset = halfHeight * (float) markDb / rangeDb;
        g.setColour (outline.withAlpha (0.45f));
        g.drawHorizontalLine ((int) (centreY - offset), area.getX(), area.getRight());
        g.drawHorizontalLine ((int) (centreY + offset), area.getX(), area.getRight());

        g.setColour (muted.withAlpha (0.35f));
        g.drawText ("+" + juce::String (markDb),
                    juce::Rectangle<float> (area.getX(), centreY - offset - 8.0f, 22.0f, 9.0f),
                    juce::Justification::centredLeft);
        g.drawText ("-" + juce::String (markDb),
                    juce::Rectangle<float> (area.getX(), centreY + offset - 1.0f, 22.0f, 9.0f),
                    juce::Justification::centredLeft);
    }
    ...
    // The band labels carry the crossover points, which are the two knobs that
    // decide what "LOW" and "HIGH" even mean here.
    auto frequency = [this] (const char* paramID)
    {
        auto* p = apvts.getParameter (paramID);
        return p != nullptr ? p->convertFrom0to1 (p->getValue()) : 0.0f;
    };
    auto asText = [] (float hz)
    {
        return hz >= 1000.0f ? juce::String (hz / 1000.0f, 1) + "k"
                             : juce::String (juce::roundToInt (hz));
    };
    const auto lowHz  = frequency (params::id::ottXoverLow);
    const auto highHz = frequency (params::id::ottXoverHigh);

    const juce::String labels[] { "LOW", "MID", "HIGH" };
    const juce::String ranges[] { "< " + asText (lowHz),
                                  asText (lowHz) + " - " + asText (highHz),
                                  "> " + asText (highHz) };
    const auto slot = area.getWidth() / 3.0f;

    for (int b = 0; b < 3; ++b)
    {
        auto column = area.withX (area.getX() + slot * (float) b).withWidth (slot);
        auto bar = column.reduced (slot * 0.28f, 0.0f);

        const auto db = juce::jlimit (-rangeDb, rangeDb, shown[(size_t) b]);
        const auto extent = halfHeight * std::abs (db) / rangeDb;

        if (extent > 0.5f)
        {
            const auto rect = db >= 0.0f
                ? juce::Rectangle<float> (bar.getX(), centreY - extent, bar.getWidth(), extent)
                : juce::Rectangle<float> (bar.getX(), centreY, bar.getWidth(), extent);
            g.setColour ((db >= 0.0f ? boostColour : cutColour).withAlpha (0.85f));
            g.fillRoundedRectangle (rect, 2.0f);
        }

        auto cell = labelRow.withX (labelRow.getX() + slot * (float) b).withWidth (slot);
        g.setColour (muted.withAlpha (0.7f));
        g.setFont (juce::Font (juce::FontOptions (8.5f, juce::Font::bold)));
        g.drawText (labels[b], cell.removeFromTop (10.0f), juce::Justification::centred);
        g.setColour (muted.withAlpha (0.4f));
        g.setFont (juce::Font (juce::FontOptions (8.0f)));
        g.drawText (ranges[b], cell, juce::Justification::centred);
    }
}
